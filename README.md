...
<p align="center">
  <b>Switch between multiple Claude Code accounts without logging out — while preserving local <code>/resume</code> session history. Bash + Python 3.</b>
</p>
...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/version-1.3.1-orange?style=for-the-badge" alt="Version">
...
2. **Local project conversation histories (`~/.claude/projects/`) remain intact.**
3. **You can start a task on Account A, switch to Account B, then start a new CLI session and run `claude --resume` or type `/resume` to resume a saved conversation when supported by the CLI and account!**
4. **No `/logout` ever happens** — your sessions and refresh tokens remain valid.


## Requirements and account safety

- Uses file-based credentials only. Respects `CLAUDE_CONFIG_DIR` (default `~/.claude`) for Claude.ai OAuth file credentials. macOS Keychain credentials and environment-provided API tokens are not switched. See [Claude credential storage](https://code.claude.com/docs/en/authentication).
- Close other CLI sessions before switching; running processes may keep or refresh their own credentials. After a manual login outside this tool, run `claude-switch save <name>` before switching so the tracking name matches the new account.
...
- Failed or interrupted login restores the previous file credentials. A successful login stays untracked until saved. Untracked credentials are preserved in `.profiles/.unsaved-*` before switching; these sensitive backups can be restored to a named profile by copying one to `.profiles/<name>/credentials.json` inside a private profile directory.
...
### 1. Save your current account under a profile name
```bash
claude-switch save [name]
```
...
| `claude-switch usage [name|all]` | `limits` | Show quota usage and reset times |
...
Because `claude-switch` **only swaps `.credentials.json`**, your project conversation history in `~/.claude/projects/` remains untouched.
...
**Yes!** Claude Code CLI stores conversation threads locally on your machine in `~/.claude/projects/`. Because `claude-switch` only swaps authentication tokens (`.credentials.json`), your local session files remain available to the CLI. You can start a conversation on one account, switch accounts with `claude-switch use [name]`, then start a new CLI session and run `claude --resume` or type `/resume` to resume a saved conversation when supported by the CLI and account!

#### Can I name and rename profiles whatever I want?
**Yes.** Names may contain ASCII letters, digits, hyphens, and underscores; `all` is reserved. For example: `claude-switch save my-label` and `claude-switch rename old-label new-label`.
...
Claude CLI can refresh access tokens during normal use. The badge uses the saved refresh-token expiry when available, otherwise the access-token expiry. An expired access token does not by itself prove the refresh token is unusable. Usage requests never renew credentials; open Claude Code on the selected profile to let the CLI renew its login, then save the profile again.
